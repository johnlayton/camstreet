      ...
      map.getLayers().forEach( function ( layer ) {
        if ( layer.get( 'dods' ) ) {
          ...
          layer.get( 'dods' ).getData = function ( x, y ) {
            ...
            var x1 = _.findLastIndex( dods[1][1], function ( i ) { return i < x } );
            var x2 = x1 + 1;
            var y1 = _.findLastIndex( dods[1][0], function ( i ) { return i < y } );
            var y2 = y1 + 1;
            return $.ajax( {
      ...
      if ( !goog.isNull( coordinate ) ) {
        ...
        if ( layer.get( 'dods' ) && layer.get('dods' ).getData ) {
          ...
          layer.get('dods').getData( coordinate[0], coordinate[1] ).then( function ( data ) {
            var dods = data[1][0];
            var values = _.map( dods[0], function( arr ) { return arr[0][0]; } );
            var dates = _.map( dods[1], function( val ) { return moment( val * 1000 ).toDate(); } );
            var d = _.filter( _.map( _.zip( dates, values ), function( arr ) { return { date : arr[0], value: arr[1] } } ), function( a ) {
            ...
            var line = d3.svg.line()
              ...
              .y(function(d) { /*debugger;*/ return y(d.value); });
