  ...
  L.Control.Clusters = L.Control.extend( {
    ...
    initialize: function ( baseLayers, overlays, options ) {
      ...
      this._clusters = L.markerClusterGroup( {
        iconCreateFunction: function ( cluster ) {
          return L.divIcon( {
            html: self.options.iconHtml ? self.options.iconHtml( cluster ) : cluster.getChildCount(),
            className: 'mycluster',
            iconSize: L.point( 90, 40 ) } );
        },
        maxClusterRadius: 120,
        disableClusteringAtZoom: 14,
        clusterProfiles: self.options.clusterProfiles ? self.options.clusterProfiles : {}
        /*
    ...
    _addItem: function ( obj ) {
      var label = document.createElement( 'label' ),
        input,
        checked = obj.overlay ? this._clusters.hasLayer( obj.layer.getLayers()[0] ) : this._map.hasLayer( obj.layer );
